...
namespace PROJECT_CAREERCIN.Controllers
{
    ...
    public class DashboardPerusahaanController : Controller
    {
        ...
        public async Task<IActionResult> CompanyProfileSetting(RegisterPerusahaanDTO dto)
        {
            ...
            FluentValidation.Results.ValidationResult validationResult = await _registerCompanyValidator.ValidateAsync(dto);
        ...
        public async Task<IActionResult> LowonganPekerjaanAddUpdate(LowonganPekerjaan lowonganPekerjaan)
        {
            ...
            FluentValidation.Results.ValidationResult validationResult = await _jobValidator.ValidateAsync(lowonganDTO);
            if (!validationResult.IsValid)
            {
                foreach (var error in validationResult.Errors)
                {
